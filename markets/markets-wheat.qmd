---
title: "Зерно"
sidebar: false
format: 
  html:
    page-layout: custom
    grid:
       margin-width: 300px
       body-width: 900px
---

# ![](../images/icon-wheat.svg) Рынок зерна {.my-custom-class style="margin-left:3%;"}

![](../images/icon-subscription.svg)[Демо-подписка](../demo-subscription.qmd)

::: {.grid style="margin-left:0%"}
::: {.g-col-12 .g-col-sm-12 .g-col-xs-12 .g-col-xl-4 .g-col-md-6 .g-col-lg-6}
### Котировки

*21.10.2024 Пшеница*

**Россия притормозила казахстанскую пшеницу**

Экспортные цены на пшеницу за неделю выросли на 8 долл.

С 17 октября 2024 г. Россия ввела временные ограничения на импорт пшеницы из Казахстана.

![](../images/icon-locker.svg){width="6%"} [Подробнее](../demo-subscription.qmd)

*14.10.2024 Пшеница*

**Алжир не ждет французскую пшеницу**

Экспортные цены на пшеницу за неделю выросли на 3 долл.

Алжир исключил французские компании из участия в тендере по закупкам пшеницы.

Reuters сообщает, что Минсельхоз РФ рекомендовал минимальную экспортную цену на пшеницу для международных тендеров - не ниже 250 долл./т FOB.

![](../images/icon-locker.svg){width="6%"} [Подробнее](../demo-subscription.qmd)

*07.10.2024 Пшеница*

**Египет решает хлебный вопрос**

Экспортные цены на пшеницу за неделю выросли на 9 долл.

Турция смягчила запрет на импорт пшеницы.

![](../images/icon-locker.svg){width="6%"} [Подробнее](../demo-subscription.qmd)
:::

::: {.g-col-12 .g-col-sm-12 .g-col-xs-12 .g-col-xl-4 .g-col-md-6 .g-col-lg-6}
#### Цена экспортной альтернативы к FOB Новороссийск пшеницы 12,5%, руб./т

```{r,warning = FALSE, echo = FALSE, message = FALSE}

library(highcharter)
library(openxlsx)
library(tidyr)

df <- read.xlsx(xlsxFile = '../xls/график_пшеница.xlsx', sheet = 1) 
df = pivot_longer(df, -Дата, names_to = "id")
names(df)[1] = 'date'
df$date <- as.Date(df$date, origin = "1899-12-30")

hc = highchart() %>%
   hc_add_series(df, type = "column", hcaes(x = date, y = value, group = id), color = c("#1A4AFC", "#FF7900"),
               name = c('Нетбэк Алтайский край', 'Нетбэк Курская область')) %>%
   hc_xAxis(type = 'datetime') %>%
   hc_yAxis(title  = list(text = 'руб./т')) %>%
   hc_exporting(enabled = FALSE) %>%
   hc_tooltip(enabled  = FALSE)

# поменять названия месяцев на русские. по умолчанию - на английском
hc$x$conf_opts$lang$months = c("Январь"	,"Февраль",	"Март"	,"Апрель"	,"Май",	"Июнь", "Июль",	"Август",	"Сентябрь",	"Октябрь",	"Ноябрь",	"Декабрь")
hc$x$conf_opts$lang$shortMonths = c("Янв", 	"Фев",	"Мар",	"Апр",	"Май",	"Июн",	"Июл",	"Авг",	"Сен",	"Окт",	"Ноя",	"Дек")

hc

```

### События

*13 сентября 2024. Пятница с ЦЦИ: хлеб с маслом*

[Материалы →](../events/2024-09-13-wheat.qmd)
:::
:::

::: {.grid style="margin-left:0%"}
::: {.g-col-12 .g-col-sm-12 .g-col-xs-12 .g-col-xl-4 .g-col-md-6 .g-col-lg-6}
### Методология

[Методология ценовых индикаторов](../methodology/methodology-benchmark-pbc.qmd)

[Спецификация котировок на пшеницу](../methodology/specs-wheat.qmd)
:::

::: {.g-col-12 .g-col-sm-12 .g-col-xs-12 .g-col-xl-4 .g-col-md-6 .g-col-lg-6}
:::
:::
